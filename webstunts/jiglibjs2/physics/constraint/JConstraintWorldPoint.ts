...
(function (jiglib) {
    ...
    JConstraintWorldPoint.prototype.apply = function (dt) {
        ...
        let worldPos = this._body.get_currentState().orientation.transformVector(this._pointOnBody);
        worldPos = worldPos.add(this._body.get_currentState().position);
        let R = worldPos.subtract(this._body.get_currentState().position);
        let currentVel = this._body.get_currentState().linVelocity.add(this._body.get_currentState().rotVelocity.crossProduct(R));
